...
TASK5="Code-Review AUFGABE 5/5 — FUNKTIONSDIMENSION 'SERVER & INFRASTRUKTUR': Analysiere die Server- und Deploy-Schicht als Betriebssystem der App: server/proxy/proxy_server.py, web/src/core/cors_fetch.ts, web/src/core/presets.ts, web/src/main.ts plus Service-Worker-Registrierung (sw.js-Logik soweit im Repo einsehbar). Flüsse: (a) PROXY-LEBENSZYKLUS: Request rein → Validierung (Allowlist, SSRF-Guard, DNS-Pinning) → Redirect-Kette → Response raus — prüfe jede Stufe einzeln und in Kombination (kann eine Kette die Guards umgehen? stimmt das Host-Header/SNI-Handling an jedem Übergang?). (b) UPDATE-FLOW: Neue Version deployed → Service-Worker-Update → alte Caches — kann der User auf einer kaputten Mischversion hängen? (c) KONFIG-FLUSS: presets.ts/cors_fetch.ts — welche URLs/Keys landen wo, funktionieren Dev- und Prod-Modus korrekt? Struktur: SECTION [DATEI:ZEILE] mit Schweregrad, Beschreibung, Fix-Vorschlag. Am Ende 3-Satz-Zusammenfassung der Infra-Robustheit.${METHOD}${STRICT}"
